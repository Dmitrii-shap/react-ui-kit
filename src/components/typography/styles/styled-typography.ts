...
export const StyledTypography = styled.div<StyledTypographyProps>`
    ...
    font-weight: ${({ theme, variant, fontWeight }) =>
        fontWeight ? fontWeightMap[fontWeight] : theme.typography[variant].fontWeight};
    ...
    text-transform: ${({ theme, variant, textTransform }) =>
        textTransform || theme.typography[variant].textTransform || 'none'};
    color: ${({ theme, variant, color }) =>
        color ? getPaletteColor(theme.palette, color) : theme.typography[variant].color || 'inherit'};
